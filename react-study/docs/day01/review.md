...
### 7. 에러 메시지 표시하기
> 에러 메시지를 언제 표시해야하는지 먼저 정의해야한다.

- 검색어 길이를 제한할 때
  - 검색어에 아무 글자도 입력하지 않는다면 검색하지 못하게 한다.
- 검색어와 일치하는 사용자가 없을 때
  - API 응답을 받아올 때 데이터가 빈 배열이면 에러 메시지를 띄운다.
- 서버 요청이 실패했을 때
  - 서버에서 성공 응답 외의 응답이 오게 되면 에러 메시지를 띄운다.
- 입력한 후 응답이 특정 시간동안 오지 않을 때

**에러 메시지를 어떤 방법으로 띄워야할까?**

에러 메시지를 띄우는 방법으로 생각한 3가지 방법이 있다.
- `인라인` 메시지로 컴포넌트 아래와 같이 근처에 에러 메시지를 보여준다.
- `토스트`를 통해 화면 상단이나 하단에 잠깐 알림을 띄워준다.
- `모달`을 통해 사용자가 바로 집중할 수 있게 보여준다.

사용자가 입력을 잘못하게 되면 보통 인라인으로 표시를 한다. 그러나 서버 응답 실패와 같은 문제는 일반적으로 토스트 메시지, 예외적으로 발생하는 중대한 시스템 에러일 경우 모달이나 전체 알림을 사용한다.

> 현재 설정한 에러 메시지 범위 내에서는 인라인이나 토스트 방식이 적절하다 판단했다.

**에러 메시지를 각 컴포넌트에서 구현하면 될까?**
> 에러 메시지를 구현하기 위해 여러 케이스의 에러 메시지를 만들어야했다.

- API 응답에 실패했을 때: API 관련 코드에서 응답이 성공했는지 확인하고 아니라면 에러 메시지를 띄운다.
- 검색어 길이 제한: 검색창에 가서 사용자가 입력한 길이를 검사하고 오류가 있다면 에러 메시지를 띄운다.

이 외에도 다양한 곳에서 에러 메시지를 띄우기 위해 에러 처리를 해주게 되면 에러 처리 코드는 다양한 곳에 분산된다. 에러 메시지를 `중앙에서 관리`할 수 있도록 하는게 필요하다 생각했다.

**에러 메시지 중앙에서 관리하기**
- 에러가 발생할 수 있는 곳에서는 어떤 에러가 발생했는지 전달만 한다.
- 애플리케이션 곳곳에서 발생한 에러를 한 곳에서 수집하여 공통된 방식으로 처리한다.
  - 공통된 방식으로 처리하게 되면, 에러 처리 방식이 변경되어도 모든 곳을 방문할 필요없이 한 곳에서 수정하면 된다.

> 중앙에서 관리하는 것이 편하다 생각했기 때문에, 인라인 메시지는 어떤 컴포넌트에 글을 표시할지 결정해야하는 복잡성이 있다 판단해 토스트나 모달 방식으로 에러를 관리하기로 했다.

1. 중앙에서 에러 관리
- Context API를 사용해 애플리케이션 최상위에서 Provider로 감싸준다.
- 

2. 에러가 발생하는 곳에서의 처리
- 에러가 발생하는 곳에서 발생한 에러가 어떤 것인지 정의한다.
- 발생한 에러 타입만 전달한다.
  - 상세 에러 메시지를 전달하지 않는 이유는 메시지를 바꾸는 것은 중앙에서 관리하는 것이 수정이 용이하다 판단했다.
  - 에러의 타입만을 전달해 언제든 해당 타입에 대한 처리를 중앙에서 수정하기 쉽도록한다.

3. 토스트 메시지 띄우기
- 전달 받은 에러 타입에 따라 에러 메시지를 설정한다.
- 에러는 연속적으로 나타날 수 있으므로 리스트로 관리해 여러 에러가 보일 수 있게 한다.
- 에러는 계속 띄우는 것이 아닌 일정 시간 후 사라지게 해야하므로 타이머 설정을 해야한다.
